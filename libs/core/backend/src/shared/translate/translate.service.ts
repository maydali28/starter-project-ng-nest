...
import {
  TRANSLATE_OPTIONS,
  TranslateOptionsModel
} from './models/translate-options.model';
...
export class TranslateService {
  ...
  private loadTranslations(): void {
    ...
    this.loadingTranslations.pipe(take(1)).subscribe(
      value => {
  ...
  private getTranslatedValue(lang: string, key: string, params?: any): string {
    const translation = this.parser.interpolate(
      this.parser.getValue(this.translations[lang], key),
      params
    );
    ...
    return this.parser.interpolate(
      this.parser.getValue(this.store.translations[lang], key),
      params
    );
  }
}
