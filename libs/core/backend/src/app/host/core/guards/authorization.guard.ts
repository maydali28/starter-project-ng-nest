...
import { OrganizationRepo } from '../../../../app/domain/organization/repo/organization.repo';
...
export class AuthorizationGuard implements CanActivate {
  ...
  public async canActivate(context: ExecutionContext): Promise<boolean> {
    try {
      ...
      const orgId: string | undefined = await this.getOrgIdFromSlug(
      ...
      for (const authcheck of authchecks) {
        ...
        this.logger.warn(
          `Checking... ${authcheck.constructor.name}: ${JSON.stringify(
        ...
        if (!(await authcheck.isAuthorized(authCheckData))) {
          this.logger.warn(
            `authcheck failed for ${
              authcheck.constructor.name
            }: ${JSON.stringify({ authcheck, authCheckData }, null, 2)}`
          );
          return false;
        }
      }
   ...
   */
  private async getOrgIdFromSlug(
  ...
  private async getauthchecksFromCache(
    context: ExecutionContext
  ): Promise<Array<AuthCheckContract<any, any>>> {
  ...
  private async getauthcheck(
    context: ExecutionContext
  ): Promise<Array<AuthCheckContract<any, any>>> {
    let authchecks = this.reflector.get<Array<AuthCheckContract<any, any>>>(
      'authorization',
      context.getHandler()
    );
    if (!authchecks) {
      authchecks = this.reflector.get<Array<AuthCheckContract<any, any>>>(
        'authorization',
        context.getClass()
      );
    }
