...
export class AttachAuthenticationHttpMiddleware implements NestMiddleware {
    ...
    private async process(req: any): Promise<void> {
        if (!req || !req.originalUrl) {
            this.loggerService.debug('req.originalUrl is null', getReqMetadataLite(req));
            return;
        }
    ...
    private async attachAuthenticatedUserToRequest(req): Promise<void> {
        ...
        const jwtPayload = verify(jwtToken, this.coreConfig.jwt.jwtPublicKey, this.verifyOptions) as IJwtPayload;
        ...
        try {
            accessPermissions = await this.accessPermissionsRepo.findById(jwtPayload.sub, { skipAuthorization: true });
            this.loggerService.debug(`Attaching authorization to request${jwtPayload.sub}`, accessPermissions);
        } catch (error) {
    ...
    private getJwtBearerTokenFromRequestHeader(req): string | undefined {
        ...
        if (!authHdr) {
            ...
            this.loggerService.debug(`No auth header found for request: ${req.originalUrl}`);
            return undefined;
        }
