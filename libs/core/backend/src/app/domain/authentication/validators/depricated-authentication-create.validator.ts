...
export class DeprecatedAuthenticationCreateValidator {
    ...
    public async validate(authenticationEntity: Authentication, options?: IAuthenticationCreateValidatorOptions) {
        ...
        if (authenticationEntity.email) {
            if (await this.authenticationRepo.findOne({ email: authenticationEntity.email }, { skipAuthorization: true })) {
