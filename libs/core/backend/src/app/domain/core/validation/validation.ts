  ...
  public async tryValidate(
    ...
    const classValidatorValidationErrors = await validate(obj, {
      skipMissingProperties:
        options.skipMissingProperties !== undefined
    ...
    if (
      ...
      classValidatorValidationErrors.length > 0
    ) {
      const validationErrorMessages: string[] = [];
      classValidatorValidationErrors.forEach(valErr => {
        validationErrorMessages.push(
          `Property '${valErr.property}' can not contain value '${
            valErr.value
          }'`
        );
      });
      validationError = new ValidationError(validationErrorMessages, {
        target: obj,
        options,
        data: classValidatorValidationErrors
      });
    }
